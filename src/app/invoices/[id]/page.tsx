...
export default function InvoiceDetailPage() {
  ...
  const inv = useQuery(api.invoices.getById, useMemo(() => ({ id }), [id])) as InvoiceDoc | undefined;
  const markPaid = useMutation(api.invoices.markPaid) as unknown as (args: { id: string }) => Promise<{ ok: boolean }>;
  const router = useRouter();
  const [loading, setLoading] = useState(false);

  if (!inv) return <main className="p-6 max-w-3xl mx-auto">Зареждане...</main>;
      ...
      <div className="border rounded-md divide-y">
        <div className="p-3 flex items-center justify-between text-sm">
          <div>
            ...
            <div className="text-muted-foreground">{inv.paid ? `Платена${inv.paidAt ? ` · ${fmtDateTimeBG(inv.paidAt)}` : ""}` : "Неплатена"}</div>
          </div>
          <div className="flex items-center gap-2">
            {inv.paid ? null : (
              <Button
            ...
            <tbody>
              {(inv.items ?? []).map((it, idx) => (
                <tr key={idx}>
                  ...
                  <td className="text-right">{fmtNumberBG(it.price, { style: "currency", currency: "BGN" })}</td>
                  <td className="text-right">{fmtNumberBG(it.total, { style: "currency", currency: "BGN" })}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr>
                <td colSpan={3} className="text-right font-medium">Общо</td>
                <td className="text-right font-medium">{fmtNumberBG(inv.total, { style: "currency", currency: "BGN" })}</td>
              </tr>
            </tfoot>
          </table>
        </div>
